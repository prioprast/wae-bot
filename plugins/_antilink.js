...
handler.before = async function (m, { isAdmin, isBotAdmin }) {
  ...
  if (chat.antiLink && isGroupLink && !isAdmin && !m.isBaileys && m.isGroup) {
    ...
    await this.sendButon(m.chat, `*Link Grup Terdeteksi, maaf kamu akan dikick dari grup!*${isBotAdmin ? '' : '\n\nUPS!, Aku bukan admin jadi gabisa ngekick t_t'}\n\nKetik *.off antilink* untuk mematikan fitur ini${opts['restrict'] ? '' : '\nketik *#on restrict* supaya bisa kick'}`, 'Goodbye dude', 'Matikan Anti link Grup', ',0 antilink', m)
